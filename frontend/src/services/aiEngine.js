...
export function getSpendingInsights(transactions, fmt) {
  ...
  if (savingsRate >= 30) {
    insights.push({ icon: '🏆', color: '#10b981', text: `Excellent! ${savingsRate.toFixed(0)}% savings rate – you're in the top tier!` });
  } else if (savingsRate >= 20) {
    insights.push({ icon: '✅', color: '#10b981', text: `Good job! ${savingsRate.toFixed(0)}% savings rate. Target 30%+ for financial freedom.` });
  } else if (savingsRate > 0) {
    insights.push({ icon: '📈', color: '#f59e0b', text: `${savingsRate.toFixed(0)}% savings rate. Try to save at least 20%.` });
  } else {
    insights.push({ icon: '⚠️', color: '#ef4444', text: `Spending exceeds income! Review your expenses immediately.` });
  }
  ...
  if (predicted !== null) {
    insights.push({
      icon: '🔮',
      color: '#7c3aed',
      text: `AI predicts ~${fmt ? fmt(predicted) : predicted.toFixed(2)} in expenses next month based on trends.`,
    });
  }
  ...
  if (cats.length >= 5) {
    insights.push({ icon: '📊', color: '#06b6d4', text: `Spending across ${cats.length} categories – good financial diversity.` });
  }
